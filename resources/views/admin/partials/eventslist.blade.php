...
<tr>
    ...
    <td class="px-6 py-4 whitespace-nowrap">{{ $event->description ? Str::limit($event->description, 50) : 'N/A' }}</td>
    <td class="px-6 py-4 whitespace-nowrap">{{ $event->ticket_price ? '$' . number_format($event->ticket_price, 2) : 'Free' }}</td>
    ...
    <td class="px-6 py-4 whitespace-nowrap">
        <a href="{{ route('Eventedit', ['id' => $event->id]) }}"
